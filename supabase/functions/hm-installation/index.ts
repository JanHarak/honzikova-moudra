...
Deno.serve(async req=>{if(req.method==='OPTIONS')return new Response(null,{headers});if(req.method!=='POST')return json({error:'METHOD'},405);try{const b=await req.json();
 ...
 if(b.action==='daily'){if(typeof b.id!=='string'||typeof b.credential!=='string'||typeof b.enabled!=='boolean'||typeof b.time!=='string'||!/^\d{2}:\d{2}$/.test(b.time))return json({error:'INVALID_INPUT'},400);check((await admin.rpc('hm_set_daily_notification',{p_id:b.id,p_hash:await sha(b.credential),p_enabled:b.enabled,p_time:b.time})).error);return json({ok:true});}
