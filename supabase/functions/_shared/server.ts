...
export async function administrator(req:Request){const u=await user(req);const {data,error}=await admin.from('hm_user_roles').select('role').eq('user_id',u.id).maybeSingle();if(error||data?.role!=='admin')throw Error('FORBIDDEN');return u;}
